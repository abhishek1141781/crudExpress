...
## postgres
CREATE DATABASE pgpromisedb_working;
\c pgpromisedb_working
CREATE TABLE users (
...
-- Create stored procedure for updating a user
CREATE OR REPLACE PROCEDURE update_user(user_id INT, name VARCHAR, email VARCHAR, OUT result BOOLEAN)
AS $$
BEGIN
    UPDATE users SET name = name, email = email WHERE id = user_id;
    IF FOUND THEN
...
-- Create stored procedure for deleting a user
CREATE OR REPLACE PROCEDURE delete_user(user_id INT, OUT result BOOLEAN)
AS $$
BEGIN
    DELETE FROM users WHERE id = user_id;
    IF FOUND THEN
